...
def main(input_files_dir):
    ...
    result = dict()
    for qwerty1 in np.linspace(0.1, 1, 100):
        for qwerty2 in np.linspace(1, 100, 1000):
            print(qwerty1, qwerty2)
            result[str(qwerty1) + "-" + str(qwerty2)] = []
            for file in file_order:
                ...
                for j in range(0, len(channel)):
                    ...
                    for i in range(0, len(channel)):
                        if channel[i] not in get_all_channels_from_group(group):
                            if np.abs(velocity[i] - chan[1]) <= qwerty1 and \
                                    np.abs(distance(ra[i], chan[4], dec[i], chan[5])) <= qwerty2:
                ...
                result[str(qwerty1) + "-" + str(qwerty2)].append(len(groups))
                '''
                for group in groups:
                    vel = []
                    inten = []
    ...
    print(min(total_results.values()))
    tmp = list(total_results.values()).index(min(total_results.values()))
    print(list(total_results.keys())[tmp])
