...
def main():
    ...
    for group in groups_indexies:
        ...
        sum_of_vel.append(sum(vel_for_group ))
        ls_tmp.append(sum(vel_for_group) / len(group))

        for index2 in range(0, len(ras)):
            ra_for_group = [ras[index2][gi] for gi in group]
            sum_of_ra.append(sum(ra_for_group))

            dec_for_group = [decs[index2][gi] for gi in group]
            sum_of_dec.append(sum(dec_for_group))

            ls_tmp.append(sum(ra_for_group) / len(group))
            ls_tmp.append(sum(dec_for_group) / len(group))

            flux_for_group = [fluxs[index2][gi] for gi in group]
            ls_tmp.append(max(flux_for_group))

        for index1 in range(1, len(ras)):
            ra_diff_tmp = [ras[index1][gi] - ras[0][gi] for gi in group]
            sum_of_ra_diff.append(sum(ra_diff_tmp))

            dec_diff_tmp = [decs[index1][gi] - decs[0][gi] for gi in group]
            ...
            ls_tmp.append(np.array(length) / len(group))
    ...
    for group_index in range(0, len(groups_indexies)):
        ellipse = Ellipse((np.mean(ls[group_index][1]), np.mean(ls[group_index][2])), angle=0, lw=0.5, width=3*np.log10(ls[group_index][11]*1000.), height=3*np.log10(ls[group_index][11]*1000.))
